...
func runGet(c *client.Client, out *output, args []string) int {
	...
	if fs.NArg() != 1 {
		fmt.Fprintln(os.Stderr, "usage: agenthub-cli get <agentName>")
		return 2
	}

	detail, err := c.GetAgent(fs.Arg(0))
	if err != nil {
		out.err(err)
		return 1
	}
